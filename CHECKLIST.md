...
- [x] 对比官方地图 vs 自建地图：官方占据范围 5.6×5.2 m / 892 格；自建 8.0×6.0 m /
      1892 格（与房间真值 8.2×6.1 m 吻合）；自建地图 vs 真值 IoU 0.239（容差 2 格 0.665），
      工具 `scripts/map_quality.py`
...
- [x] 初始位姿（坐标 = spawn 点 -2.0/-0.5）通过 `initial_pose_x/y/a` 参数直接指定，
      重启导航即在起点，粒子云与激光轮廓对齐
- [x] 依次设 3 个目标点，记录到达与耗时（`scripts/nav_goal_test.py`，自动记录）：
      西间北侧 (-1.0,1.5) **13.0 s** → 东间（穿门洞）(2.0,0.5) **18.0 s** →
      西间南侧 (-3.2,-2.2) **33.6 s**，**3/3 到达**
- [x] 绕障验证：运行中途插入 0.8 m 箱子（`spawn_model`）挡在 (-3.2,-2.2)→(-1.0,1.5)
      的连线上；全局代价地图确认其周围出现 **233 个致命格（cost=100）**，
      同一目标点仍到达（**27.1 s**，无障碍基线 27.0 s）——局部规划器实时避障生效
- [ ] 截图存档：粒子云收敛前后、激光对齐瞬间、重规划瞬间

#### A4 录制脚本与博客

- [ ] 按 `blog/VIDEO_CHECKLIST.md` 分镜录 30 秒视频：环境展示 → 建图 → 导航 → 总结
- [ ] 截图/视频素材归档到 `blog/assets/`
- [x] 博客加"自主搭建"章节（博客 01 第 10 节）：环境设计、流程、量化结果、四个新坑
- [ ] 勾掉本里程碑，素材计入复试检查单
...
- [ ] 里程碑 A：自建环境建图/导航截图 + 30 秒视频（`blog/assets/`）
- [ ] 每个模块 3~5 分钟演示视频（B 站）
