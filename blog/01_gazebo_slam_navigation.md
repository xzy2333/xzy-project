...
官方参数不是为这个 8×6 m 小房间调的，所以我做了五组对照：同一世界、同一条
85 秒固定路线（`scripts/drive_lab.py`），只改参数，比地图质量（`scripts/map_quality.py`
算与真值栅格的容差 IoU）：

| 组 | 改动 | 更新帧 | 容差 IoU | 覆盖范围 | 占据格 |
|---|---|---|---|---|---|
| A 官方默认 | — | 169 | 0.432 | 7.5×6.0 m | 1012 |
| B 更细更新 | `linearUpdate 1.0→0.4`、`angularUpdate 0.2→0.1` | 200 | 0.426 | 7.3×6.0 m | 1042 |
| C 更宽容匹配 | `minimumScore 50→30`、`maxUrange 3.0→3.5` | 171 | **0.452** | **8.0×6.0 m** | 1096 |
| D 让运动触发主导 | C + `temporalUpdate 0.5→2.0` | 73 | 0.420 | 7.9×6.0 m | 956 |
| E 细阈值 + 运动触发 | D + `linearUpdate 0.4`、`angularUpdate 0.1` | 108 | **0.452** | 7.9×5.9 m | 1020 |

（房间真值 8.2×6.1 m；五组都没有丢帧告警。）

三个结论：

1. **放宽匹配门槛 + 用满量程（C）收益最大**——`minimumScore 50→30`、
   `maxUrange 3.0→3.5` 后覆盖补齐到 8.0×6.0 m、IoU 0.432→0.452。官方门槛对
   小房间偏严，"分数不够就丢帧"是地图缺边的主因。
2. **B 组没收益，D/E 组把这个原因证实了**——B 只调细了距离/角度阈值，但
   `temporalUpdate=0.5 s` 在主导（扫描 2 Hz，几乎每帧都触发时间更新），所以设置
   不生效。把 `temporalUpdate` 放宽到 2.0 s 让"走了多少米"真正决定更新（D 组，
   更新帧从 169 降到 73）后，**再调细阈值立刻见效**（E 组 IoU 0.420→0.452）。
   这就是一个典型的"**参数被另一个参数掩盖**"的坑，也是这套实验最有价值的发现。
3. **E 与 C 打平但更省**——同为 0.452，E 只用 108 次更新、C 用了 171 次；
   C 多出的更新多由"原地微动"的时间触发带来，收益很低。所以推荐 **E 组**
